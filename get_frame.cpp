...
struct VideoFrame {
    AVFrame* ptr;  // não-owning pointer (lifetime gerenciado externamente)
...
inline AVPixelFormat pixel_format(VideoFrame f) {
    return f.ptr ? static_cast<AVPixelFormat>(f.ptr->format) : AV_PIX_FMT_NONE;
}
...
inline bool open(VideoDecoder& dec, const char* path) {
    ...
    if (avformat_open_input(&dec.format_context, path, nullptr, nullptr) < 0)
        return false;
    ...
    for (unsigned i = 0; i < dec.format_context->nb_streams; ++i) {
        if (dec.format_context->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    const AVCodec* codec = avcodec_find_decoder(
        dec.format_context->streams[dec.video_stream_index]->codecpar->codec_id
    );
    
    if (!codec) return false;
    
    dec.codec_context = avcodec_alloc_context3(codec);
    if (!dec.codec_context) return false;
    
    if (avcodec_parameters_to_context(
            dec.codec_context,
            dec.format_context->streams[dec.video_stream_index]->codecpar) < 0)
        return false;
...
namespace std {
    template<>
    struct iterator_traits<VideoFrameIterator> {
...
inline bool operator==(const VideoFrameIterator& a, const VideoFrameIterator& b) {
 ...
 *  
 *  Algoritmos em EOP são funções matemáticas sobre conceitos abstratos.
 *  Não dependem de tipos concretos, apenas de operações garantidas.
 * ======================================================================== */
...
// Complexidade: O(n)
template <typename I>
I advance_n(I i, typename std::iterator_traits<I>::difference_type n) {
...
nth_element_or_default(I first, I last,
...
// nth_element: versão sem default (retorna valor na posição ou invalido)
template <typename I>
typename std::iterator_traits<I>::value_type
nth_element(I first, I last, typename std::iterator_traits<I>::difference_type n)
{
...
inline RGBFrame to_rgb(VideoFrame src) {
    if (!is_valid(src)) return RGBFrame{};
    
    SwsContext* sws = sws_getContext(
        width(src), height(src), pixel_format(src),
        width(src), height(src), AV_PIX_FMT_RGB24,
        SWS_BILINEAR, nullptr, nullptr, nullptr
    );
    
    if (!sws) return RGBFrame{};
    ...
    sws_scale(sws, src.ptr->data, src.ptr->linesize, 0, height(src),
...
{
    if (argc != 4) {
        std::cerr << "uso: " << argv[0] << " video.mp4 frame_index output.ppm\n";
        return EXIT_FAILURE;
    }
    ...
    if (!is_valid(target)) {
        std::cerr << "erro: frame " << frame_index << " não encontrado\n";
        close(decoder);
        return EXIT_FAILURE;
    }
    ...
    if (success) {
        std::cout << "frame " << frame_index << " salvo em " << output_path << '\n';
        return EXIT_SUCCESS;
    } else {
