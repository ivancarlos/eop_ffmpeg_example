Segue uma versão “corrigida” e segura, mantendo seu estilo, mas:

* **corrige o bug de ponteiro danificado** (não retornamos mais um frame já liberado);
* separa melhor responsabilidades:
  `main` abre/fecha o vídeo, o algoritmo genérico **só** anda pelos frames;
* cada `read()` devolve um `AVFrame*` novo, que o chamador deve liberar.
...
//   - bool   open()
//   - Frame* read()   // retorna ponteiro "dono" do frame (caller libera)
//   - void  close()
...
AVFrame* get_nth_frame(Src& src, std::size_t n)
{
    ...
    for (std::size_t i = 0; i <= n; ++i) {
        AVFrame* next = src.read();   // alocado dentro de Src::read()
        if (!next) {
    ...
    return current;  // caller é dono e deve dar av_frame_free(&current)
}
    ...
    AVFrame* read()
    {
        while (av_read_frame(fmt_, pkt_) >= 0) {
            ...
            if (ret == AVERROR(EAGAIN)) {
                // decoder precisa de mais dados; descarta esse frame e continua
                av_frame_free(&frame);
                continue;
            }
            if (ret == AVERROR_EOF) {
...
Se você quiser, no próximo passo a gente pode:

* deixar `VideoFile` **não copiável** explicitamente,
* ou evoluir para algo mais “Stepanov” ainda, com um “iterator de frames” (`FrameIterator`) e `get_nth` trabalhando sobre iteradores / ranges.
